...
from eencijferho.utils.dec_validation import read_dec_validation_log

_console = Console()


def _print_dec_validation_summary(logs_dir: str) -> None:
    """Print a clear summary of DEC validation results to the CLI."""
    log_path = os.path.join(logs_dir, "(5c)_dec_validation_log_latest.json")
    failures = read_dec_validation_log(log_path)
    if not failures:
...
def cmd_convert(args: argparse.Namespace) -> None:
    ...
    _print_dec_validation_summary(logs_dir)
