...
# A key that satisfies load_key's MIN_KEY_BYTES floor. pseudonymize_value takes
# raw bytes and skips that check, so the short _KEY above stays valid for it.
_VALID_KEY = "g" * MIN_KEY_BYTES
...
def test_load_key_explicit():
    assert load_key(key=_VALID_KEY) == _VALID_KEY.encode("utf-8")


def test_load_key_from_file(tmp_path):
    f = tmp_path / "key.txt"
    f.write_text(f"  {_VALID_KEY}\n", encoding="utf-8")
    assert load_key(key_file=str(f)) == _VALID_KEY.encode("utf-8")


def test_load_key_from_env(monkeypatch):
    monkeypatch.setenv(ENCRYPT_KEY_ENV, _VALID_KEY)
    assert load_key() == _VALID_KEY.encode("utf-8")


def test_load_key_explicit_beats_env(monkeypatch):
    monkeypatch.setenv(ENCRYPT_KEY_ENV, "e" * MIN_KEY_BYTES)
    assert load_key(key=_VALID_KEY) == _VALID_KEY.encode("utf-8")
...
def test_load_key_too_short_raises():
    with pytest.raises(ValueError, match="te kort"):
        load_key(key="g" * (MIN_KEY_BYTES - 1))


def test_load_key_exact_minimum_accepted():
    key = "g" * MIN_KEY_BYTES
    assert load_key(key=key) == key.encode("utf-8")


def test_load_key_short_multibyte_raises():
    # A non-ASCII char is multiple UTF-8 bytes, but still under the floor.
    with pytest.raises(ValueError, match="te kort"):
        load_key(key="é" * 16)
...
def test_replaces_bsn_in_place(tmp_path):
    ...
    pseudonymize_files(output_dir=str(tmp_path), key=_VALID_KEY)
    ...
    assert values[0] == pseudonymize_value(_VALID_KEY.encode("utf-8"), "111111111")
...
def test_all_three_sensitive_columns_replaced(tmp_path):
    ...
    df = _read_csv(tmp_path / "EV2023.csv")
    expected_key = _VALID_KEY.encode("utf-8")
    assert df[DUO_BSN_COLUMN][0] == pseudonymize_value(expected_key, "111111111")
    assert df[DUO_PGN_COLUMN][0] == pseudonymize_value(expected_key, "PGN001")
    assert df[DUO_ONDERWIJSNUMMER_COLUMN][0] == pseudonymize_value(expected_key, "ON001")
...
def test_processes_vakhavw_file(tmp_path):
    ...
    assert df[DUO_BSN_COLUMN][0] == pseudonymize_value(_VALID_KEY.encode("utf-8"), "555555555")
...
def test_file_without_sensitive_columns_skipped(tmp_path):
    ...
    log = pseudonymize_files(output_dir=str(tmp_path), key=_VALID_KEY)
...
def test_key_never_appears_in_log(tmp_path):
    ...
    secret = "super-geheime-sleutel-xyz-" + "z" * 64
